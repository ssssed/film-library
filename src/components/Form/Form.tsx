...
const Form: FC<IForm> = ({ films, handleUpdateFilms }) => {
  ...
  const [ageRating, setAgeRating] = useState<"18+" | "16+" | "12+" | "0+">(
    "0+"
  );
  ...
  const handleGenreChange = (e: React.ChangeEvent<HTMLSelectElement>) =>
    setGenre(
      Array.from(e.target.selectedOptions, (option) => option.value as Genre)
    );
  ...
  const handleAgeRatingChange = (e: React.ChangeEvent<HTMLSelectElement>) =>
    setAgeRating(e.target.value as "18+" | "16+" | "12+" | "0+");
  ...
  return (
    <form className="form" onSubmit={handleSubmitForm}>
      ...
      <input
        ...
        value={compositor}
        onChange={handleCompositorChange}
        required={true}
      />
      <input
        className="form__input"
